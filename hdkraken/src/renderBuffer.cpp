...
HdKrakenRenderBuffer::HdKrakenRenderBuffer(SdfPath const& id)
    : HdRenderBuffer(id)
    , _width(0)
    , _height(0)
...
HdKrakenRenderBuffer::~HdKrakenRenderBuffer() = default;

/*virtual*/
void
HdKrakenRenderBuffer::Sync(HdSceneDelegate *sceneDelegate,
                           ...
                           HdDirtyBits *dirtyBits)
{
    if (*dirtyBits & DirtyDescription) {
        ...
        //static_cast<HdKrakenRenderParam*>(renderParam)->AcquireSceneForEdit();
    }
...
/*virtual*/
void
HdKrakenRenderBuffer::Finalize(HdRenderParam *renderParam)
{
    // Embree has the background thread write directly into render buffers,
...
/*virtual*/
void
HdKrakenRenderBuffer::_Deallocate()
{
...
/*static*/
size_t
HdKrakenRenderBuffer::_GetBufferSize(GfVec2i const &dims, HdFormat format)
{
...
/*static*/
HdFormat
HdKrakenRenderBuffer::_GetSampleFormat(HdFormat format)
{
...
/*virtual*/
bool
HdKrakenRenderBuffer::Allocate(GfVec3i const& dimensions,
                               HdFormat format,
                               bool multiSampled)
{
    _Deallocate();
...
void
HdKrakenRenderBuffer::Clear(size_t numComponents, float const* value)
{
    size_t formatSize = HdDataSizeOfFormat(_format);
...
void
HdKrakenRenderBuffer::Clear(size_t numComponents, int const* value)
{
    size_t formatSize = HdDataSizeOfFormat(_format);
    for (size_t i = 0; i < _width*_height; ++i) {
        uint8_t *dst = &_buffer[i*formatSize];
